...
$(function(){
    ...
    if(userid!=null){
        $.getJSON("http://127.0.0.1:5500/v1/info",{userid:userid},function(data){
            if(data.data == null){
            ...
            // console.log(skill['keywords'].split(" "));
            let edus = new Array();
            edus = data.data.edu;
            ...
            specialtys = data.data.specialty;
...
function changeSpecialtys(specialtys){
    ...
    // console.log(specialtys[0]);
    let specialtys_0_rows = specialtys[0].description.split("\n");
    for(let i=0;i<specialtys_0_rows.length;i++){
        let $n = specialtys_0_rows[i]+"<br>";
        $(".specialty1-description").append($n);
    }

    // console.log(specialtys_0_rows);

    $(".specialty1-name").text(specialtys[0].name);
    // $(".specialty1-description").text(specialtys[0].description);

    let specialtys_1_rows = specialtys[1].description.split("\n");
    for(let i=0;i<specialtys_1_rows.length;i++){
        let $n = specialtys_1_rows[i]+"<br>";
        $(".specialty2-description").append($n);
    }
